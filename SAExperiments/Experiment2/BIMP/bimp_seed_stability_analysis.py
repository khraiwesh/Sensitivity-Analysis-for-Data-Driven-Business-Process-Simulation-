"""Seed-pair rank-stability analysis for BIMP only, per dataset x gateway
config, sourced directly from BIMP_sobol_cycletime_extract.xlsx.

Same methodology as prosimos_seed_stability_analysis.py: per (dataset, gw),
builds seed-level rankings at each sample size N, computes seed-pair rank
agreement (fraction of concordant parameter pairs), and finds the smallest N
at which the ranking is stable across all seed pairs and stays stable for
all larger N. Only cases == 1000 rows are used (matches project convention
for BIMP cycle-time results).
"""
import itertools
...
XLSX = Path(r"C:\Users\Samira\SAExperiments\Experiment2\BIMP\BIMP_sobol_cycletime_extract.xlsx")
OUT_DIR = XLSX.parent / "bimp_seed_stability"
BIMP_CASES = 1000
# a handful of "Without gateway" folders exist with n_samples=64 (stray
# exploratory runs, not part of the main 512/1024/2048 Sobol design) -> exclude.
VALID_N_SAMPLES = [512, 1024, 2048]
...
def main():
    df = pd.read_excel(XLSX, sheet_name="BIMP_sobol_cycletime")
    df = df[(df["cases"] == BIMP_CASES) & (df["n_samples"].isin(VALID_N_SAMPLES))].copy()
