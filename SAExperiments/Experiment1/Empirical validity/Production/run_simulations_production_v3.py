"""
run_simulations_production_v3.py
-----------------------------------
Runs Prosimos simulations for the Production individual-group perturbed
models created by create_models_production_v3.py.

Only the 5 individual-group models are simulated (RC/AD/RN/TR/AC), plus
the original (unperturbed) model as baseline -- same structure as
run_simulations_2012_v3.py / _2013_v3.py / _2017_v3.py.

Output goes to simulation_outputs/production_v3/

Run:
    conda activate thesis_env
    python3 run_simulations_production_v3.py
"""
...
# EDIT THESE two paths if your files live somewhere else
BPMN_PATH = os.path.expanduser("~/Desktop/thesis_eval/Production_train.bpmn")
ORIGINAL_JSON_PATH = os.path.expanduser("~/Desktop/thesis_eval/Production_train.json")

# where create_models_production_v3.py saved the perturbed model_*.json files
MODELS_DIR = os.path.expanduser("~/Desktop/thesis_eval/")

OUTPUT_DIR = os.path.expanduser("~/Desktop/thesis_eval/simulation_outputs/production_v3/")

TOTAL_CASES = 3000  # matches Samira's SA runs (see Production README)
SEEDS = [1, 2, 3, 4, 5]

MODELS = [
    ("original", None),  # handled specially below (uses ORIGINAL_JSON_PATH)
    ("RC", "model_RC_production_v3.json"),
    ("AD", "model_AD_production_v3.json"),
    ("RN", "model_RN_production_v3.json"),
    ("TR", "model_TR_production_v3.json"),
    ("AC", "model_AC_production_v3.json"),
]
...
def run_one_simulation(model_name, json_path, seed, logs_dir, kpis_dir):
    ...
    try:
        run_simulation(
            ...
            starting_at="2020-01-01T00:00:00+00:00",
        )
...
def main():
    print("=" * 60)
    print("Production (v3, quantile-based) -- Running simulations")
